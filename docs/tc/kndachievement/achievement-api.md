...
成就系統提供以下信號，可用於監聽成就相關事件並執行自定義邏輯：

### achievement_unlocked

**信號簽名：** `achievement_unlocked(achievement_id: String, data: Dictionary)`

**觸發時機：** 當任何成就被解鎖時觸發

**參數說明：**
- `achievement_id`: 被解鎖的成就ID
- `data`: 成就的完整資料字典，包含名稱、描述、圖示等資訊

**使用情境：** 用於在成就解鎖時執行獎勵邏輯、播放慶祝動畫、顯示自定義通知等
...
**信號簽名：** `achievement_progress_updated(achievement_id: String, current: float, target: float)`
...
**使用情境：** 用於顯示成就進度條、提供進度回饋、更新 UI 顯示等
...
**信號簽名：** `achievements_reset()`

**觸發時機：** 當所有成就被重設時觸發

**參數說明：** 無參數

**使用情境：** 用於在成就重設後更新 UI、顯示重設通知、執行清理操作等
...
**信號簽名：** `achievements_loaded()`

**觸發時機：** 當成就系統完成載入時觸發

**參數說明：** 無參數

**使用情境：** 用於在成就載入完成後初始化 UI、執行依賴於成就資料的邏輯等
