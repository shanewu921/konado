...
變數系統允許在腳本中定義、讀取、修改和判斷變數，實現動態對話內容、條件分支和狀態追蹤。變數值可以在對話文本中直接引用，也可以作為條件判斷的依據來控制劇情走向。
...
| 操作 | 說明 | 範例 |
|------|------|------|
| `set` | 設置變數值 | `set %love = 10` |
| `add` | 加法（數值相加，字串拼接） | `add %love 5` |
| `sub` | 減法 | `sub %love 3` |
| `mul` | 乘法 | `mul %love 2` |
...
| 值 | 是 | `10` | 整數、浮點數、布林值（`true`/`false`）或字串（雙引號包裹） |
...
在對話文本中直接使用 `%變數名` 或 `$變數名` 引用變數值，執行時會被替換為實際值。

### 語法

```
"角色名" "對話文本，包含 %變數名 或 $變數名"
```
...
"Kona" "你的好感度是 %love，當前是第 $round 回合。"
```
...
`else:` 區塊為可選，省略時條件不成立則跳過整個 `if` 區塊。
...
2. 條件判斷**不支援巢狀**，即 `if` 區塊內不能再包含 `if`；
3. 多個獨立的條件判斷應使用平鋪的 `if` / `endif` 結構，而非巢狀；
...
結合 `choice` 和 `branch`，可以在用戶做出選擇後修改變數值，實現選擇影響後續劇情。
...
在編輯器中建立 `KND_VariableStore` 資源，在檢查器中設置初始變數值，然後賦值給 `KND_DialogueManager` 的 `variable_store` 屬性。
...
臨時變數無需預設，在腳本中首次使用 `set` 時自動建立。切換鏡頭時自動重置。
...
1. **變數名**只能包含字母、數字和下劃線，區分大小寫；
2. **持久變數**（`%`）的值會隨存檔保存，適合記錄好感度、劇情標記等跨鏡頭狀態；
3. **臨時變數**（`$`）在切換鏡頭時自動清空，適合記錄當前鏡頭內的臨時狀態；
4. **除法操作**時除數為零會觸發錯誤並跳過該操作；
5. **條件判斷**不支援巢狀，多個條件請使用平鋪的 `if` / `endif` 結構；
